...
- Each task must include:
  ...
  3) target word count (100-350 words)
...
def run_workers_sequentially(
    ...
    for task in plan.tasks:
        ...
        print(f"Finished section: {task.title}")

    return sections

# -----------------------------
# Sequential generation node
# -----------------------------

def sequential_generation_node(state: State):

    plan = state["plan"]

    sections = run_workers_sequentially(
        topic=state["topic"],
        mode=state["mode"],
        plan=plan,
        evidence=state.get("evidence", []),
...
g.add_node("sequential_generation", sequential_generation_node)
